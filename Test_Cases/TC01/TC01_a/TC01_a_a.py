...
# Browser setup is kept inside each test so every scenario can also run independently.
...
file_name = "TC01_a_a.csv"

# Each function represents one user-facing checkpoint in the mobile-plan purchase journey.
# SafeStep receives the click target, destination heading, report label, and exact expected copy.
...
def Go_To_PlanSelection_Page():
    ...
    SafeStep(ps.XPATH_1MONTH_PLAN_BTN,0,0,0)

def Go_to_PlanOffer_Page():
    SafeStep(ps.XPATH_CONTINUE_BTN,po.XPATH_HEADER,"Plan Offer","Change to 12 months, pay upfront and save 50% off your plan")

def Go_to_SelectNumber_Page():
    SafeStep(po.XPATH_STAY_MONTHLY_BTN,sn.XPATH_HEADER,"Select Number","Find a number you love")
...
# Execute the journey in the same order a customer follows it on the website.
Confirm_Main_Page()
Go_to_JoinPage()
...
# Main mobile plan flow - 1 Month - Stay on 1 Month - Order
